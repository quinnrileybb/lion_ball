...
# ── Logo: fetch once from URL ──────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def load_logo():
    url = "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Penn_State_mark.svg/800px-Penn_State_mark.svg.png"
    try:
...
def draw_psu_header(fig, batter_name, game_label, logo_img=None, stats_line=None):
    ...
    if logo_img is not None:
        logo_h_in   = 1.6
        logo_h_frac = logo_h_in / FIG_H
        logo_aspect = logo_img.shape[1] / logo_img.shape[0]
        logo_w_frac = logo_h_frac * logo_aspect * (FIG_H / FIG_W)
        pad = 0.005
        for x0 in [pad, 1.0 - logo_w_frac - pad]:
            lax = fig.add_axes([x0, 0.87, logo_w_frac, logo_h_frac])
            lax.imshow(logo_img)
            lax.axis('off')
...
def build_hitter_figures(batter_df, batter_name, game_label, logo_img=None):
    ...
    gs  = GridSpec(3, 4, figure=fig,
                   width_ratios=[1.4, 1.4, 1.4, 1.8],
                   height_ratios=[1, 1, 1],
                   left=0.02, right=0.99, bottom=0.11, top=0.84,
    ...
    for i, (pa_id, pa_data) in enumerate(plate_appearance_groups, start=1):
        ...
        ax.set_title(f'PA {i} vs {handedness}', fontsize=15, fontweight='bold', color=PSU_NAVY)
        ax.text(0.5, -0.13, f'P: {pitcher_name}', fontsize=9, fontstyle='italic',
    ...
    fig.legend(handles=handles1, title='Pitch Call (Colors)',
               loc='lower center', bbox_to_anchor=(0.38, 0.04),
               ncol=len(unique_calls), fontsize=10, title_fontsize=11, frameon=False)
    fig.legend(handles=handles2, title='Pitch Type (Shapes)',
               loc='lower center', bbox_to_anchor=(0.38, 0.01),
               ncol=len(PITCH_TYPE_MARKERS), fontsize=10, title_fontsize=11, frameon=False)

    # Pitch-by-pitch table (rightmost column)
    ...
    for row in table_data:
        if y_pos < 0.01:
            break
        if 'PA' in row[0] and row[1] == '':
            ax_table.text(0.02, y_pos, row[0], fontsize=12, fontweight='bold',
                          fontstyle='italic', color=PSU_NAVY, va='top')
            ax_table.axhline(y=y_pos - 0.005, xmin=0.0, xmax=1.0,
                             color=PSU_NAVY, linewidth=0.8)
            y_pos -= 0.040
        else:
    ...
    # ── Batted ball page ───────────────────────────────────────────────────────
    bb_fig = plt.figure(figsize=(22, 13), facecolor='white')

    bb_ax = bb_fig.add_axes([0.02, 0.09, 0.55, 0.75])
    ...
    bb_ax.set_xticks([])
    bb_ax.set_yticks([])
    bb_ax.set_xlim(-310, 310)
    bb_ax.set_ylim(-25, 435)
    ...
    bb_fig.legend(handles=legend_els, loc='lower center', bbox_to_anchor=(0.29, 0.01),
                  ncol=5, fontsize=10, frameon=False)

    # Stats panel
    stats_ax = bb_fig.add_axes([0.60, 0.09, 0.38, 0.75])
    stats_ax.axis('off')
